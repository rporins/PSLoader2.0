/**
 * ACCPAC WORKSHEET IMPORT PROCESSOR
 * ==================================
 *
 * This processor handles the import of ACCPAC worksheet data from CSV files.
 * It validates and processes worksheet items for integration into the system.
 *
 * @version 1.0.0
 */
 ...
 * ACCPAC Worksheet Import Processor
 *
 * This processor imports ACCPAC worksheet data from CSV files.
 * It validates the data structure and processes each worksheet item
 * according to business rules.
 */
export class AccpacWorksheetImportProcessor extends BaseImportProcessor {
  /**
   * STEP 1: DEFINE METADATA
   */
  metadata: ImportProcessorMetadata = {
    // Unique identifier
    id: 'accpac_worksheet_import',

    // Display name shown in the UI
    name: 'ACCPAC Worksheet Import',

    // Detailed description of what this import does
    description: 'Imports ACCPAC worksheet data from CSV files. Validates and processes worksheet items including account codes, descriptions, and period amounts.',
    ...
    validationRules: [
      'CSV file must contain all required columns: FILETYPE, FILEVERSION, PROPCODE, BUSDATE, ACCTCODE, ACCTDESC, WORKSHEETITEMCODE, PERIODAMT',
      'PROPCODE in first data row must match hotel Local ID 1',
      'BUSDATE must match the selected import period (format: YYYYMM)'
    ],

    // Tags for filtering/searching
    tags: ['accpac', 'worksheet', 'financial'],
  ...
  protected async processRows(parsed: ParsedFile, options?: ImportOptions): Promise<ImportResult> {
    ...
    try {
      ...
      // Generate import batch ID: "accpac_worksheet_import_YYYY-MM-DD_HH-MM-SS"
      const now = new Date();
      const batchId = `accpac_worksheet_import_${now.toISOString().replace(/[:.]/g, '-')}`;
      ...
      // Step 3: NO truncation - we simply add to the staging table (no clearStagingTable call)

      // Step 4: Group rows by mapped combo and aggregate PERIODAMT
      ...
      return {
        ...
        metadata: {
          ...
          batchId,
          message: `ACCPAC worksheet import completed successfully`,
          mappingStats: {
            total: processedRows,
            mapped: mappedRows,
  ...
  /**
   * Aggregate rows by mapped combo and sum PERIODAMT amounts
   * Handles multiple source rows mapping to the same target combo
   * Uses config_id = 10 for mappings
   * Maps ACCTCODE column to source_account in mappings
   */
  private async aggregateAndMapRows(
    ...
    for (const row of rows) {
      ...
      const periodAmt = parseFloat(row.PERIODAMT) || 0;
      ...
      // Skip rows with zero amount (only process non-zero data)
      if (periodAmt === 0) {
        continue;
      }

      // Lookup mapping using ACCTCODE -> source_account
