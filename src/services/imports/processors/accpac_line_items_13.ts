/**
 * ACCPAC LINE ITEMS IMPORT PROCESSOR (MAPPING CONFIG 13)
 * =====================================================
 *
 * Variant of the ACCPAC Line Items processor for a hotel that uses the
 * SAME file format but a DIFFERENT mapping table (mapping_config_id = 13).
 *
 * The ETL logic is identical to accpac_line_items.ts — only the mapping
 * config id differs. See accpac_line_items.ts for the canonical implementation.
 *
 * @version 1.0.0
 */
 ...
 * according to business rules, applying mapping config 13.
 */
export class AccpacLineItems13Processor extends BaseImportProcessor {
  /**
   * STEP 1: DEFINE METADATA
   */
  metadata: ImportProcessorMetadata = {
    // Unique identifier
    id: 'accpac_line_items_13',

    // Display name shown in the UI
    name: 'ACCPAC Line Items (Config 13)',

    // Detailed description of what this import does
    description: 'Imports ACCPAC line items from CSV files using mapping config 13. Validates and processes line item data for integration into the system.',
  ...
  protected async processRows(parsed: ParsedFile, options?: ImportOptions): Promise<ImportResult> {
    ...
    try {
      ...
      // Generate import batch ID: "accpac_line_items_13_YYYY-MM-DD_HH-MM-SS"
      const now = new Date();
      const batchId = `accpac_line_items_13_${now.toISOString().replace(/[:.]/g, '-')}`;
  ...
  private async aggregateAndMapRows(
    ...
    // Get all mappings for config_id = 13
    const mappings = await db.getMappings(13);
    // console.log(`[${this.metadata.id}] Loaded ${mappings.length} mappings for config_id = 13`);
